...
1. **`Ctrl + Arrow` local passthrough** (macOS) — per-session toggle in the
   remote toolbar's keyboard menu. See below.
2. **Always start remote session maximized** — global option in the **main
   window** Settings → Display, next to "Open connection in new tab"
   (`kOptionStartRemoteMaximized` = `"start-remote-maximized"`). On by default.
   When a new remote-desktop session window opens, it is maximized
   (`restoreWindowPosition()` in `flutter/lib/common/common.dart`, both the
   saved-position and no-saved-position paths, gated to
   `WindowType.RemoteDesktop`). Because the option key is not prefixed with
   `enable-`/`allow-`, an unset value resolves to `true`, so the default is
   maximized; unchecking stores `"N"`. It lives in the main window (not a
   per-session toolbar) because it governs how *future* connections open.
...
- `OPTION_CTRL_ARROW_LOCAL` (`"ctrl-arrow-local"`) — session toggle option key.
- `is_ctrl_arrow_local_enabled()` — reads the current session's toggle option
  (mirrors `get_keyboard_mode()`'s session lookup). Off by default.
...
- The passthrough block runs only when the hook is active, the key is a
  Ctrl/Arrow key, and the option is enabled — so the per-keystroke session
  lookup is skipped for all other keys.
...
### Flutter UI
- `flutter/lib/consts.dart` — `kOptionCtrlArrowLocal = "ctrl-arrow-local"`
  (must match the Rust constant).
- `flutter/lib/common/widgets/toolbar.dart` — `toolbarKeyboardToggles()` adds a
  checkbox in the keyboard menu when the **local** machine is macOS. It reuses
  the generic session toggle plumbing (`sessionGetToggleOptionSync` /
  `sessionToggleOption`), so persistence is automatic and no `client.rs` changes
  are needed (the generic option store defaults unset → off).
...
- **Local side effect:** while enabled, the local OS also sees `Ctrl` presses
  ...
  feature is opt-in per session.
- **Scope:** macOS client only; off by default; per-session toggle.
